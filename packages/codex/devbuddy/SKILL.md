...
- Change shared behavior, role scope, constraints, output expectations, workflows, capabilities, references, or memory policy in this skill folder first.
- Change `agents/shared/` before changing platform adapters when a role's behavior should apply across adapters.
- Change `agents/codex/` or `agents/openai.yaml` only for Codex-specific routing, metadata, or prompt mechanics.
