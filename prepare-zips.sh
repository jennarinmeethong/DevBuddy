...
rm -f "$CODEX_ZIP" "$CLAUDE_ZIP"

if command -v zip >/dev/null 2>&1; then
  (cd "$PACKAGES/codex" && zip -qr "$CODEX_ZIP" devbuddy)
  (cd "$CLAUDE_SRC" && zip -qr "$CLAUDE_ZIP" .claude skill)
else
  POWERSHELL_BIN=""
  ...
  "$POWERSHELL_BIN" -NoProfile -ExecutionPolicy Bypass -Command \
    ...
    "Compress-Archive -LiteralPath '$CLAUDE_AGENTS_PS','$CLAUDE_SKILL_PS' -DestinationPath '$CLAUDE_ZIP_PS' -CompressionLevel Optimal;"
fi
